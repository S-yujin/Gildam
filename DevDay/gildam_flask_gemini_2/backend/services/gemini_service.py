...
def _load_master_df() -> pd.DataFrame:
    ...
    address = col(df, "주소 기타", "주소", "장소", "address")
...
class GeminiService:
    ...
    def _pick_model_name(self) -> str:
        """
        현재 API 키로 호출 가능한 v1 모델을 조회하여
        generateContent를 지원하는 것을 우선순위대로 선택.
        """
        ...
        try:
            ...
            # 메서드 이름 필드가 릴리즈에 따라 달라질 수 있어 두 키 모두 확인
    ...
    # --- v1 REST 호출 유틸 (Google AI Studio 키용) ---
    def _rest_generate_content(self, prompt: str) -> str:
        """
        Google AI Studio 키 기반 v1 REST 호출.
        - x-goog-api-key 헤더 사용 (쿼리스트링 대신)
        - ListModels로 사용가능 모델 자동 선택
        - 404 발생 시 다른 후보 모델로 자동 재시도
        """
        api_key = Config.GEMINI_API_KEY
        headers = {
        ...
        # 모델 이름은 v1 REST에서 보통 "models/..." 풀네임을 사용
        def to_full_name(name: str) -> str:
            return name if name.startswith("models/") else f"models/{name}"

        # 재시도 후보 목록 만들기 (선호순 + pick 결과 우선)
        preferred = [
            self.model_name,
            "models/gemini-2.5-flash",
            "models/gemini-2.0-flash",
        ...
        for cand in preferred:
            ...
            try:
                ...
                if r.status_code == 404:
                    logger.warning(f"   ⚠️ 404 Not Found: {name} → 다음 후보 시도")
                ...
                logger.error(f"   ❌ {name} 호출 실패: {e}")
                errors.append(str(e))
                continue
            except Exception as e:
                logger.error(f"   ❌ {name} 예외: {e}")
                errors.append(str(e))
                continue

        raise RuntimeError(f"모든 모델 호출 실패. 시도: {len(tried)} / 에러: {errors}")
    ...
    def generate_itinerary(self, trip_data: Dict[str, Any]) -> Union[Dict, None]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # --- Gemini 호출 (SDK 우선, 실패 시 REST 폴백) ---
                logger.info("   🤖 Gemini 호출 중...")
                response_text = None

                if self._use_sdk:
                    # SDK는 "models/" 접두사 없이 이름 사용
                    if not self.model_name:
                        self.model_name = self._pick_model_name()
                    ...
                    try:
                        ...
                        em = str(e)
                        logger.error(f"   ❌ SDK 호출 실패: {em}")
                        logger.info("   🔁 REST(v1)로 폴백 호출")
                        response_text = self._rest_generate_content(prompt)
                        logger.info(f"   ✅ REST 응답 (길이: {len(response_text)}자)")
                else:
                    response_text = self._rest_generate_content(prompt)
                    logger.info(f"   ✅ REST 응답 (길이: {len(response_text)}자)")
    ...
    def _validate_itinerary(self, data: Dict[str, Any], trip_data: Dict[str, Any]) -> bool:
        ...
        for i, day_plan in enumerate(itinerary, 1):
            if not isinstance(day_plan, dict):
                logger.error(f"      ❌ {i}일차가 dict가 아님")
                return False

            places = day_plan.get("places", [])
            if not isinstance(places, list) or not places:
                logger.error(f"      ❌ {i}일차 장소가 비어있음")
                return False
            ...
            for j, place in enumerate(places, 1):
                ...
                if not hhmm_ok(place["start_time"]) or not hhmm_ok(place["end_time"]):
                    logger.error(f"      ❌ {i}일차 {j}번째 장소의 시간 형식 오류")
                    return False
    ...
    def _get_fallback_itinerary(
        self, trip_data: Dict[str, Any], candidates: pd.DataFrame
    ) -> Dict[str, Any]:
        """API 실패 시 기본 일정 제공"""
        logger.warning("🔄 Fallback 일정 생성")
